...
Version : 
Date    : 01/11/2020
Comments: Contador de 0 a 9, sin rebotes de push button
...
void main(void){
    ...
    while( 1 ){
        if( boton == 0 )
            ...
            boton_actual = 1;    
        if( ( boton_pasado == 1 ) && ( boton_actual == 0 ) ){
            cont++;
            if( cont == 10 )
                cont = 0;
            delay_ms( 40 );
        }
        if( ( boton_pasado == 0 ) && ( boton_actual == 1) )
            delay_ms( 40 );
        display = contador[cont];
        boton_pasado = boton_actual;
    }
}
