...
bit izq_actual, izq_pasado, der_actual, der_pasado, at_actual, at_pasado, ad_actual, ad_pasado;
...
void atras();
void adelante();
void generar_signal( unsigned int fr );
...
void izquierda(){
    if( !B_IZQ ){
        izq_actual = 0;
        generar_signal( 100 );
        if( !izq_pasado &&  izq_actual )
            delay_ms( 20 );
    }
    else
        izq_actual = 1;
    izq_pasado = izq_actual;
}
...
void derecha(){
    if( !B_DER ){
        der_actual = 0;
        generar_signal( 200 );
        if( !der_pasado &&  der_actual )
            delay_ms( 20 );
    }
    else
        der_actual = 1;
    der_pasado = der_actual;
}
